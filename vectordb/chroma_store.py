...
class ChromaStore:
    def __init__(self, collection_name="chunks", persist_dir=None, reset=False):
        if persist_dir:
        ...
        if collection_name in existing:
            # Reopen the persisted collection instead of wiping it — otherwise
            # every query-time ChromaStore() (app.py, the agent) would erase
            # what ingest.py just built. Pass reset=True to rebuild on purpose.
    ...
    def add(self, chunks, embeddings):
        ...
        for m in metadatas:
            ...
            for k, v in m.items():
                if v is None:
                    continue  # Chroma rejects None; drop instead of stringifying to "None"
